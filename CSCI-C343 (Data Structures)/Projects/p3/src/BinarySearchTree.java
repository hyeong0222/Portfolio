...
public class BinarySearchTree<K> implements Tree<K> {
  ...
  public Node insert(K key) {
	  ...
	  else if (root.data == key && root.dirty) {
		  n++;
		  root.dirty = false;
		  contain = true;
	  }
	  while (node != null & contain == false) {
		  if (lessThan.test(root.data, key)) {
			  if (node.left == null) {
				  n++;
			  ...
			  else if (node.left.data == key && node.left.dirty == true) {
				  n++;
			  ...
			  else if (node.right.data == key && node.right.dirty == true) {
				  n++;
