...
public class Util {
  ...
  public static double cosineSimilarity(ColorTable A, ColorTable B) {
    return (dotProduct(A,B) / (vectorMag(A) * vectorMag(B)));
  }
  ...
  public static double vectorMag (ColorTable A) {
	  ...
	  while (iter.hasNext()) {
		  vectorMag += Math.pow(iter.next(), 2);
	  }
   ...
   */
  	public static Color unpack(int code, int bitsPerChannel) {
  		...
  		else throw new RuntimeException("Unsupported number of bits per channel; use an int in the range [1..8]");    
  	}
