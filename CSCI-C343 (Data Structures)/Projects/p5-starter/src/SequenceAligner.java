...
public class SequenceAligner {
  ...
  public static Result bestResult(int diag, int left, int up) {
	  ...
	  else return new Result(up, Direction.UP);
  ...
  private void fillCache() {
	  ...
	  for (int i = 1; i < cache.length; i++) {
		  for (int j = 1; j < cache[i].length; j++) {
			  int diag = cache[i - 1][j - 1].getScore() + judge.score(this.getX().charAt(i - 1), this.getY().charAt(j - 1));
			  int left = cache[i][j - 1].getScore() + judge.score(Constants.GAP_CHAR, this.getY().charAt(j - 1));
			  int up = cache[i - 1][j].getScore() + judge.score(this.getX().charAt(i - 1), Constants.GAP_CHAR);
			  
			  cache[i][j] = bestResult(diag, left, up);
		  }
	  }
  ...
  private void traceback() {
	  ...
	  cache[a][b].markPath();
	  Result current = cache[a][b];
	  while (current.getParent() != Direction.NONE) {
		  if (current.getParent() == Direction.DIAGONAL) {
			  x.append(this.getX().charAt(a - 1));
			  y.append(this.getY().charAt(b - 1));
			  current = cache [a - 1][b - 1];
			  a--;
			  b--;
		  }
		  else if (current.getParent() == Direction.LEFT) {
			  x.append(Constants.GAP_CHAR);
			  y.append(getY().charAt(b - 1));
			  current = cache[a][b - 1];
			  b--;
		  }
		  else {
			  x.append(getX().charAt(a - 1));
